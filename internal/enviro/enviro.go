...
func (svc *Service) InterveneAround(trialID int64, at time.Time, before, after time.Duration) (Intervention, error) {
	...
	// BUG: the first row is assumed to be the warm baseline even when storage returns newest first.
	// BUG: the first row is treated as the baseline even when storage returns newest first.
	iv.Cooling = model.CoolingDetected(samples[0].TempC, minT) && samples[0].SampledAt.Before(at)
	return iv, nil
}
