...
// CountEnv returns the number of persisted environment samples.
func (s *Store) CountEnv(trialID int64) (int, error) {
	var count int
	if err := s.db.QueryRow(`SELECT COUNT(*) FROM env_samples WHERE trial_id=?`, trialID).Scan(&count); err != nil {
		return 0, err
	}
	return count, nil
}
